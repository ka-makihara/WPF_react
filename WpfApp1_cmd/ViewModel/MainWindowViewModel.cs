...
*/
		...
		///  転送中止
		/// </summary>
		private async void StopTransferExecute()
		{
		...
		public async Task<bool> UploadModuleFiles(LcuInfo lcu, MachineInfo machine, ModuleInfo module, CancellationToken token)
		{
			...
			try
			{
				...
				ret = await MergeUpdateCommon(lcu, machine, module);
			}
			catch (Exception e)
			{
				Debug.WriteLine("UploadModuleFiles Error");
				throw;
			}
		...
		private async Task<bool> MergeUpdateCommon(LcuInfo lcu, MachineInfo machine, ModuleInfo module)
		{
			...
			var lines = System.IO.File.ReadAllLines(path, Encoding.GetEncoding(Define.TXT_ENCODING));
